...
modal = Modal(key="Demo Key", title="Selected Idea", padding=20,  max_width=744)
...
if uploaded_file and openai_api_key:
    ...
    with top_menu[1]:
        sort_field = st.selectbox("Sort By", options=df_metrics.columns, index=df_metrics.columns.get_loc('overall_score'), key='sort_field_2')
    with top_menu[2]:
    ...
    def get_row_and_clear_selection():
        ...
        with modal.container():
            data_query = st.dataframe(df.iloc[selected_rows])
        try:
